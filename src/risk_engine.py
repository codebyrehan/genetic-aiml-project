...
@lru_cache(maxsize=1)
def get_model():
    """Train the deterministic educational Random Forest once per process."""
...
def analyze(payload: dict) -> dict:
    """Return a transparent risk signal for the supported synthetic feature space."""
    row = {
    ...
    if not 0 <= row["mismatches"] <= 6:
        raise ValueError("Mismatch count must be between 0 and 6.")
    if not 0 <= row["conservation_score"] <= 1:
        raise ValueError("Conservation score must be between 0 and 1.")
    if not 0.3 <= row["gc_content"] <= 0.7:
        raise ValueError("GC content must be between 0.3 and 0.7.")
