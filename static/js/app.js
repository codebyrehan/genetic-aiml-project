...
async function updateDashboard(genetic) {
  ...
  try {
    ...
    if (!ecoResponse.ok) throw Error(eco.error || 'Ecological assessment failed');
    ecoScore = Number(eco.score ?? eco.risk_score ?? eco.ecological_score ?? 0);
  ...
  try {
    ...
    if (!regResponse.ok) throw Error(reg.error || 'Regulatory assessment failed');
    const regScore = Number(reg.regulatory_signal ?? reg.score ?? reg.readiness_score ?? reg.regulatory_score ?? 0);
...
$('report-btn').addEventListener('click', async () => {
  ...
  try {
    ...
    $('result-state').textContent = 'REPORT ERROR';
    $('risk-copy').textContent = err.message;
  }
});
...
function buildRiskMap() {
  ...
  script.onerror = () => { $('genova-map').innerHTML = '<div class="map-fallback">Map library unavailable. Synthetic risk zones remain documented in the project methodology.</div>'; };
  document.body.appendChild(script);

  let initialized = false;
  function initMap() {
    if (initialized || !window.L || !document.getElementById('genova-map')) return;
    initialized = true;
    const map = L.map('genova-map', {scrollWheelZoom: false}).setView([20, 15], 2);
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {attribution: '&copy; OpenStreetMap contributors'}).addTo(map);
    const zones = [[28,77,'Synthetic Zone A','Lower signal · conservation proxy 0.42'],[0,30,'Synthetic Zone B','Review zone · population-impact proxy 0.61'],[-25,135,'Synthetic Zone C','Higher signal · ecological sensitivity 0.84'],[35,-100,'Synthetic Zone D','Review zone · uncertainty proxy 0.57']];
